...
func (provider *Provider) ReceiveMessage(ctx context.Context, sender peer.ID, incoming message.BitSwapMessage) {
	...
	for _, entry := range incoming.Wantlist() {
		...
		stats.Record(ctx, metrics.BitswapMessagesReceivedCount.M(1))
		...
		if provider.config.CIDBlacklist[entry.Cid] {
			...
			// TODO: add metric count for blacklisted CID
			provider.sendDontHave(ctx, sender, entry)
			continue
		}
...
func (provider *Provider) sendDontHave(ctx context.Context, sender peer.ID, entry message.Entry) {
	ctx, _ = tag.New(ctx, tag.Insert(metrics.BitswapTopic, "DONT_HAVE"))
